...
export default function ArticleInfo(props) {
  ...
  return (
    <div className={style.article_info}>
      ...
      <Form onFinished={handleAdd}>
        <Form.Item label="题图">
          <div
            className={style.pic_wrapper}
            onMouseEnter={handleShowMask}
            onMouseLeave={handleHideMask}
          >
            {visible && (
              <div className={style.pic_mask}>
                <PlusOutlined />
              </div>
            )}
            <img className={style.title_pic} src={picSrc} />
          </div>
        </Form.Item>
        ...
        <Form.Item label="简介" name="introduce">
          <Input.TextArea placeholder="请用一句话来概括..." />
        </Form.Item>
